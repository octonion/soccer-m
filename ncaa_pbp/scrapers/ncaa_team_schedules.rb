...
ncaa_team_schedules << [
  ...
  "score_string", "error", "exempted",
  "team_won", "score", "team_score", "opponent_score",
      ...
      doc.search(game_xpath).each do |game|
        ...
        game.search("td").each_with_index do |element,k|
          ...
          when 2
            ...
            score_string = element.text.scrub.strip
            
            if score_string.include?(" *")
              score_cleaned = score_string.gsub(" *","")
              exempted = TRUE
            else
              score_cleaned = score_string
              exempted = FALSE
            end

            if score_cleaned.include?(" ERROR")
              score_cleaned = score_cleaned.gsub(" ERROR","")
              error = TRUE
            else
              error = FALSE
            end
            
            score_parameters = score_cleaned.split(" ",2)
            ...
            row += [score_string, error, exempted, team_won, score, team_score, opponent_score, overtime, overtime_periods, game_id, game_url]
          end
        end
